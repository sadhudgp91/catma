...
class RWTagger:
    ...
    def convert_txtfile_to_dateframe(self, txt_file):
        ...
        result = {"tok": [], "tokenstart": [], "tokenend": []}
        # nltk sentence splitter
        sent_detector = nltk.data.load("tokenizers/punkt/german.pickle")
        
        # Load Spacy Module to detect tokens and write token start and End Indices
        nlp = spacy.load('de_core_news_sm')
        with open(txt_file, "r", encoding="utf8") as f:
            content = "".join(f.readlines())
        sentences = sent_detector.tokenize(content.strip())
        offset, length = 0, 0
        file_doc = nlp(content)
        
        for token in file_doc:  
            result["tok"].append(token.text)
            result["tokenstart"].append(token.idx)
            result["tokenend"].append(token.idx + len(token.text) - 1)
